...
class TXBA_Utilities {
  ...
  parseFavoriteHtml(html) {
    ...
    return this.parseFavoriteData($(".accordion-title"));
  }
  ...
  getFavForms() {
    const code =
      "7766%7C4302%7C5527%7C5285%7C4957%7C8497%7C8386%7C8337%7C8303%7C8234";
    ...
    // .then(response => parseCriteria(response));
  }
  ...
  getSearchEntries(code, auth) {
    const url = `${this.baseURL}${this.search_slug}/${code}/${auth}${this.slug_code[code]}`;
    // console.log(url)
    return axios
      .get(url)
      .then(async response => await response.data) // .then((response) => console.log(response))
  ...
  async parseSearchResults(html) {
    const $ = cheerio.load(html);
    return this.getSearchFilterInfo($("div[id^=browserResultItem]"));
  }

  getSearchFilterInfo(group) {
